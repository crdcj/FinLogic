...
def search_segment(search_value: str):
    series = LAST_SESSION_DF["segment"].drop_duplicates().sort_values(ignore_index=True)
...
def search_company(
    ...
    df = pd.merge(df, LAST_SESSION_DF, on="cvm_id")
    match search_by:
        case "name_id":
...
def rank(
    ...
    df = (
        ...
        .merge(LAST_SESSION_DF, on="cvm_id")
